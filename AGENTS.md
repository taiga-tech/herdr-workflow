...
変更は必要なコード、テスト、主文書に限定する。同じ仕様を複製せず、目次で指定した主文書を更新する。文書ID、機能ID、試験ID、ADR番号を振り直さない。元資料と移行対応表は`archive/`と`docs/meta/`に保存されており、現行仕様として編集しない。一回限りの検査結果は永続参照文書へ含めず、CI、作業記録、Git履歴で確認する。

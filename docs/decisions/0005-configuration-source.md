...
YAML、Rust型、JSON Schema、文書中の完全な例を別々に更新すると、型や参照先の説明が一致しなくなる。元文書の設定例には全機能の入力形式がまだ定義されていない。
...
実装前の現時点ではRust型も生成スキーマも存在しない。文書検査の合格を設定スキーマへの適合と呼ばない。旧設定例は`archive/`に保存し、現行例の編集を妨げない。

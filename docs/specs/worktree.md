...
コピー先の重複判定では、相対パスの`.`要素、連続する区切り、末尾の区切りを字句的に正規化し、パス要素単位で同一・親子関係を検出する。`.env`と`./.env`、`config/`と`config/local.env`、`config/local.env`と`config//local.env`はいずれも重複として拒否する。

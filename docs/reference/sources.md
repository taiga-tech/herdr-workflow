...
| `herdr-plugin.toml`は`id`/`name`/`version`/`min_herdr_version`が必須で、`[[actions]]`/`[[panes]]`/`[[startup]]`/`[[events]]`/`[[link_handlers]]`/`[[build]]`のセクションを持つ。[S1](sources.md#s1)[S2](sources.md#s2) | manifest形式の型定義とCLIのaction/pane宣言方法へ反映する。                          |
...
| 資料ID | 対象release                            | 実行環境                          | 確認日                 | 確認内容                                                                                                                                                                                                                                                              | 試験結果                                                                                                                                                                                                                 |
| ------ | -------------------------------------- | --------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| S1・S2 | Herdr v0.9.0(インストール済みバイナリ) | macOS(x86_64-apple-darwin)、Linux | 2026-09-14、2026-09-16 | `herdr api schema --json`でSocket APIスキーマを取得し、`InstalledPluginInfo`型と`PluginManifest*`系の型定義を確認。v0.9.0タグのplugins.mdx([S2](sources.md#s2)のGitHub raw版)と記載内容を突き合わせた。Linuxではclient/serverの版とprotocol、既定設定、pane CLIも確認 | 実機スキーマと公開ドキュメントの記載が一致。マニフェストの必須フィールド・各セクション構造を確認。公開設定とAPIスキーマにはpane数上限を確認できず、内部限界は未確認。`min_herdr_version`の対応下限そのものは未決定のまま |

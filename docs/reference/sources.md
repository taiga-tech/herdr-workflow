...
| ------ | -------------------------------------- | -------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1・S2 | Herdr v0.9.0(インストール済みバイナリ) | macOS(x86_64-apple-darwin) | 2026-09-14 | `herdr api schema --json`でSocket APIスキーマを取得し、`InstalledPluginInfo`型と`PluginManifest*`系の型定義を確認。v0.9.0タグのplugins.mdx([S2](sources.md#s2)のGitHub raw版)と記載内容を突き合わせた | 実機スキーマと公開ドキュメントの記載が一致。マニフェストの必須フィールド・各セクション構造を確認。`min_herdr_version`の対応下限そのものは未決定のまま |

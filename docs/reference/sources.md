...
S1〜S14は元文書の参考資料を移管した。元文書に記載された公開資料確認日は2026-09-11であり、今回の分割で全リンク先の内容を再検証したという意味ではない。製品の互換性を主張するときは、対象版と実機試験を別に記録する。

D1〜D3は今回の文書構成を検討するために2026-09-11に確認した一次資料。分類、ADR、リンクの扱いを参照した。外部資料の内容から、このプラグインの実装済み機能を推定しない。
